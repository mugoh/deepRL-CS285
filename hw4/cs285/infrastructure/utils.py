...
def calculate_mean_prediction_error(env, action_sequence, models,
    ...
    # predict states using the model and given action sequence and
    # initial state
    ...
    for ac in action_sequence:
        ...
        # TODO(Q1) Get predicted next state using the model
        ob = model.get_prediction(obs, action, data_statistics)
    pred_states = np.squeeze(pred_states)

    # Calculate the mean prediction error here
    mpe = model.update(true_states,
                       action_sequence,
                       pred_states,
                       data_statistics)  # TODO(Q1)
...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        rollout_done = done or steps >= max_path_length  # : GETTHIS from HW1
