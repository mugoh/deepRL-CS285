...
def build_mlp(input_placeholder, output_size, scope, n_layers, size,
    ...
    """
        ...
        arguments:
            input_placeholder: placeholder variable for the
        ...
        returns
            output_placeholder: the result of a forward pass
                through the hidden layers + the output layer
    """
    output_placeholder = input_placeholder
    with tf.variable_scope(scope):
        for _ in range(n_layers):
            # HINT: use tf.layers.dense
            # (specify <input>, <size>, activation=<?>)
...
def create_tf_session(use_gpu, gpu_frac=0.6, allow_gpu_growth=True,
                      which_gpu=0):
    if use_gpu:
        # gpu options
