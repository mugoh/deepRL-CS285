...
def get_env_kwargs(env_name, lander_spec_name=None):
    if env_name == 'PongNoFrameskip-v4':
        kwargs = {
            'learning_starts': 50000,
            'target_update_freq': 10000,
            'replay_buffer_size': int(1e6),
            'num_timesteps': int(4e8),
            'q_func': atari_model,
            'learning_freq': 4,
        ...
        kwargs['exploration_schedule'] = lander_exploration_schedule(
            kwargs['num_timesteps'])
        kwargs['optimizer_spec'] = lander_optimizer() if not lander_spec_name \
            else globals()[
                lander_spec_name](kwargs['num_timesteps']),
        opt = kwargs['optimizer_spec']

        kwargs['optimizer_spec'] = opt[0] if isinstance(
            kwargs['optimizer_spec'], tuple) else opt
...
def lander_optimizer_annealed_2(num_timesteps):
    num_iterations = num_timesteps/4
    lr_multiplier = 1.0
    lr_schedule = PiecewiseSchedule([
...
def lander_optimizer_annealed_3(num_timesteps):
    num_iterations = num_timesteps/4
    lr_multiplier = 1.0
    lr_schedule = PiecewiseSchedule([
        (0, 1e-4 * lr_multiplier),
...
def lander_optimizer_annealed_1(num_timesteps):
    num_iterations = num_timesteps/4
    lr_multiplier = 1.0
    lr_schedule = PiecewiseSchedule([
